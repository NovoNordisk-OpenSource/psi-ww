# Generate corrected LSM_Score data based on publication figures
# Reference: HMER-17-61 publication figures
...
summary_stats <- full_data %>%
  ...
  summarise(
    ...
    Pct_Change = round(((mean(EOS_kPa) - mean(Baseline_kPa)) / mean(Baseline_kPa)) * 100, 1)
  )
print(summary_stats)
...
# Save to CSV
write.csv(full_data, "202509/LSM_Score_corrected.csv", row.names = FALSE)

cat("\n✓ Data saved to LSM_Score_corrected.csv\n")
cat("Review the data and if it looks good, replace LSM_Score.csv with this file.\n")
