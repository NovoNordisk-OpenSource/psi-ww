...
(pp1 <- ggplot() +
  ...
  labs(
    ...
    x = "Study Day",
    y = "Daily glucocorticoid dose (mg/kg/day)"
  ) +
  theme_minimal())
...
(pp2 <-
  ...
  labs(
    title = "Line Plot per Subject",
    x = "Study Day",
    y = "Weekly average glucocorticoid dose"
  ) +
  theme_minimal())
...
dummy_points <- data.frame(
  ...
  threshold_color = factor(
    c("<0.2", "0.2–0.5", "0.5–1.0", "≥1.0"),
    levels = levels(mmean_data$threshold_color)
  )
)

# Plot with threshold-specific colors
(mean_gc_dose_plot <- ggplot(
  mmean_data,
  aes(x = AVISIT, y = mean_dose, group = 1)
) +
...
filtered_df <- df |>
  mutate(
    dose_threshold_mg_kg = cut(
      AVAL1,
      breaks = bins_clinical,
      labels = labels_clinical,
      right = FALSE
    )
  )
...
grouped_df_clinical <- grouped_df_clinical %>%
  mutate(
    dose_threshold_mg_kg = factor(
      dose_threshold_mg_kg,
...
pivot_df <- filtered_df |>
  ...
  pivot_wider(
    id_cols = SUBJID,
    names_from = AVISITW,
    values_from = dose_threshold_mg_kg
  )
...
create_sort_key <- function(df, visit_cols) {
  ...
  df <- df |>
    mutate(across(all_of(visit_cols), ~ ifelse(is.na(.), NA, as.character(.))))
...
# Convert the pivot table to long format for plotting without converting NA values
plot_df <- pivot_df |>
  pivot_longer(
...
# Replace NA levels with "Missing" and set factor levels including "Missing"
plot_df <- plot_df |>
  mutate(
    SUBJID = factor(SUBJID, levels = rev(sorted_subjects)),
    dose_threshold_mg_kg = fct_explicit_na(
      dose_threshold_mg_kg,
      na_level = "Missing"
    ),
    dose_threshold_mg_kg = factor(
      dose_threshold_mg_kg,
      levels = rev(c("<0.2", "0.2–0.5", "0.5–1.0", "≥1.0", "Missing"))
    )
  )

# Create the heatmap with "Missing" labeled in the legend
(heatmap_plot <- ggplot(
  plot_df,
  aes(x = AVISITW, y = SUBJID, fill = dose_threshold_mg_kg)
) +
  geom_tile() +
  scale_fill_manual(
    values = c(
  ...
  labs(
    title = "Subject Dose Threshold Shifts Over Time",
    x = "Week",
    y = "Subject ID",
    fill = "Dose Threshold"
  ) +
