...
# Updated function to create a consistent color scale for AE categories
create_ae_color_mapping <- function(ae_list) {
  n_colors <- length(ae_list)
  if (n_colors < 3) {
    # For 1 or 2 colors, use a different color palette
    color_palette <- c("#E41A1C", "#377EB8", "#4DAF4A")[1:n_colors]
  } else {
    color_palette <- brewer.pal(min(n_colors, 9), "YlOrRd")
  }
